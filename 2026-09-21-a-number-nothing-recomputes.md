...
I already had `verify_evidence.py`: 21 claims, each with a public source, re-checked live on
...
```
prose counts checked:    11 (truth: 21 claims, 17 live, 4 on request)
```
...
- Deriving a count means the prose stops being readable on its own. `21 claims, 17 live` is
